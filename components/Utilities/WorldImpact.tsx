        ...
        <div className="relative w-full max-w-6xl aspect-[1.8/1] md:aspect-[2/1] bg-[#48C0B2] rounded-xl">
          ...
          {mapPins.map((pin, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{
                delay: Math.min(idx * 0.02, 2), // cap delay so later pins don't take forever
                type: "spring",
                stiffness: 260,
                damping: 20,
              }}
              className="absolute -translate-x-1/2 -translate-y-1/2 z-30 hover:z-50 group cursor-pointer"
            ...
            >
              ...
              <div className="absolute -top-8 md:-top-10 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-giq-dark backdrop-blur-sm text-giq-light text-[10px] md:text-xs font-bold px-2 py-1 rounded-md whitespace-nowrap pointer-events-none shadow-xl border border-white/20 z-50">
                {pin.label}
