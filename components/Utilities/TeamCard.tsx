...
export default function TeamCard({ data, isAdvisor }: { data: Person, isAdvisor?: boolean }) {
      ...
      <div className="relative bg-white/80 backdrop-blur-md border border-white/60 p-8 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 h-full flex flex-col">
        ...
        <div className="text-center flex-grow">
          ...
          {!isAdvisor && data.country && (
            <span className="inline-block bg-white border border-gray-200 text-gray-600 text-xs px-3 py-1 rounded-full mb-4 shadow-sm">
              📍 {data.country}
            </span>
          )}
          
          <p className="text-gray-600 text-sm leading-relaxed line-clamp-4 group-hover:line-clamp-none transition-all duration-300">
            {isAdvisor ? data.desc : (data.bio || "Passionate about Quantum Computing.")}
          </p>
        </div>
